// bot.js - LPZX Bot v4 - Discord Lua Tools (ATUALIZADO)
...
const OWNER_ID = '1472747077883658315'; // ID do Dono
...
// Lista para .suported (SEM LUARMOR)
        ...
        try {
            ...
            if (service.name === 'Pastefy') {
                response = await axios.post(service.url, {
                    content, title: title || 'LPZX Upload', syntax: 'lua', visibility: 'PUBLIC'
                }, {
                    headers: { 'Authorization': `Bearer ${service.key}` },
                    timeout: 10000
                });
                if (response.data?.id) return {
                    service: service.name, id: response.data.id,
                ...
                if (response.data?.key) return {
                    service: service.name, id: response.data.key,
                ...
                response = await axios.post(service.url, {
                    ...
                    sections: [{ name: 'lua', syntax: 'lua', contents: content }]
                }, {
                    headers: { 'X-Auth-Token': service.key }, timeout: 10000
                });
                if (response.data?.id) return {
                    service: service.name, id: response.data.id,
                    url: service.format(response.data.id).url,
            ...
            else if (service.name === 'GitHub Gist' && service.key) {
                response = await axios.post(service.url, {
                    description: title || 'LPZX Upload',
                    public: true,
                    files: { [title || 'script.lua']: { content } }
                }, {
                    headers: { 'Authorization': `token ${service.key}` }, timeout: 10000
                });
                if (response.data?.id) {
...
async function handleDump(message, args, statusMsg) {
    ...
    try {
        ...
        let pasteField = { name: 'Paste', value: 'Upload failed', inline: false };
        if (pasteResult) {
            addUploadRecord(message.author.id, 'dump', pasteResult.url);
            const shortUrl = await shortenUrl(pasteResult.url);
            pasteField = { name: `📋 ${pasteResult.service}`, value: `[Link](${shortUrl}) | [RAW](${pasteResult.raw})`, inline: false };
        }
        ...
        if (parseFloat(elapsedTime) > timeout * 0.8) {
            embed.addFields({ name: '⚠️ Warning', value: `Near timeout limit (${timeout}s)`, inline: false });
        }
...
async function handleUpload(message, args, statusMsg) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: 'Lines', value: `${formatted.split('\n').length}`, inline: true },
            { name: 'Size', value: `${(formatted.length / 1024).toFixed(2)}KB`, inline: true },
            { name: `📋 ${pasteResult.service}`, value: `[Link](${shortUrl}) | [RAW](${pasteResult.raw})`, inline: false }
        )
...
async function handleHelp(message, args, statusMsg) {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '.upload <URL/file>', value: 'Upload to paste services', inline: false },
...
client.on('messageCreate', async (message) => {
    ...
    if (userId === OWNER_ID) {
        if (command === 'verify') {
            ...
            if (target) { await setCredits(target.id, 999, true); return message.reply(`✅ ${target.username} verified!`); }
        }
        if (command === 'setcredits') {
            const target = message.mentions.users.first();
            const amount = parseInt(args[1]);
            if (target && !isNaN(amount)) { await setCredits(target.id, amount); return message.reply(`✅ Set ${amount} credits for ${target.username}`); }
        }
        if (command === 'remove') {
            const target = message.mentions.users.first();
            if (target) { await setCredits(target.id, 0, false); return message.reply(`❌ Removed permissions for ${target.username}`); }
        }
    }

    // Verificação de Créditos para comandos principais
    const mainCommands = ['dump', 'bf', 'get', 'renamer', 'decompiler', 'compress', 'minify'];
    if (mainCommands.includes(command)) {
        const credits = await getUserCredits(userId);
        if (!credits.verified) return message.reply(`⚠️ Você precisa se verificar no site primeiro!\n🔗 ${API_URL}`);
        if (credits.amount <= 0) {
        ...
        if (!result.success) return message.reply('Erro ao processar créditos.');
