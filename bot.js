...
function formatLuaCode(code) {
    ...
    for (let i = 0; i < formatted.length; i++) {
        if (formatted[i] === '' && (i === 0 || formatted[i-1] === '')) {
            continue; // Pular linhas em branco duplicadas
        }
...
async function handleDump(message, args, statusMsg) {
    ...
    try {
        ...
        // FORMATAR O CÓDIGO (organizar com quebras de linha)
        ...
        // SEMPRE enviar arquivo .lua completo
        const fileName = `dump_${Date.now()}.lua`;
        const attach = new AttachmentBuilder(Buffer.from(formatted), { name: fileName });
        
        // Preview no embed (primeiras 10 linhas apenas)
        const previewLines = formatted.split('\n').slice(0, 10).join('\n');
...
async function handleMinify(message, args, statusMsg) {
    ...
    try {
        ...
        // FORMATAR O CÓDIGO (mas minify já deve estar minificado)
...
async function handleCompress(message, args, statusMsg) {
    ...
    try {
        ...
        // FORMATAR (opcional, mas compress já é comprimido)
...
async function handleHelp(message, args, statusMsg) {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '.upload <URL/file>', value: 'Upload to Pastefy (formats code)', inline: false },
