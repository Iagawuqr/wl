    ...
    for (const ua of userAgents) {
        try {
            ...
            if (content.includes('<!DOCTYPE') || content.includes('<html') || 
                ...
                console.log(`[WGET] Conteúdo inválido (HTML ou muito pequeno)`);
            ...
            console.log(`[WGET] Download concluído: ${content.length} bytes`);
...
async function handleHelp(message, args, statusMsg) {
    ...
    // Comandos de admin (só aparecem para o dono)
    if (isOwner) {
        embed.addFields(
            { name: '👑 **ADMIN COMMANDS**', value: '─────────────', inline: false },
            { name: '.verify @user', value: 'Verificar usuário (999 créditos)', inline: false },
            { name: '.setcredits @user <amount>', value: 'Definir créditos manualmente', inline: false },
            { name: '.remove @user', value: 'Remover permissões do usuário', inline: false }
        );
    }
    
    embed.addFields(
        { name: '📋 Formats', value: '.lua .txt .luac .luc .bytecode', inline: false },
        { name: '⚠️ Limits', value: `• Max file size: 20MB\n• Uploads per day: ${MAX_UPLOADS_PER_DAY}\n• Timeout: 30s`, inline: false },
