...
// FUNÇÃO CORRIGIDA - USA WGET DO TERMINAL
async function fetchWithWget(url) {
    ...
    for (const ua of userAgents) {
        attempts++;
        try {
            console.log(`[WGET] Attempt ${attempts} with UA: ${ua.substring(0, 50)}...`);
            ...
            if (content.includes('<!DOCTYPE') || 
                ...
                console.log(`[WGET] Attempt ${attempts} returned error HTML`);
                fs.unlinkSync(outputFile);
                continue; // Tentar próximo User-Agent
            }

            // Se chegou aqui, deu certo!
            console.log(`[WGET] Success on attempt ${attempts}! Size: ${content.length} bytes`);
            fs.unlinkSync(outputFile); // Limpar arquivo temporário
            return content;

        } catch (error) {
            console.log(`[WGET] Attempt ${attempts} failed:`, error.message);
            lastError = error;
...
// FUNÇÃO CORRIGIDA - Upload to Pastefy com retry
async function uploadToPastefy(content, title) {
    // Tentar até 3 vezes
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            console.log(`[PASTEFY] Attempt ${attempt} for ${title}`);
            ...
            if (response.data && response.data.id) {
                console.log(`[PASTEFY] Success: ${response.data.id}`);
                stats.total_pastes++;
                return {
                    id: response.data.id,
            ...
            console.error(`[PASTEFY] Attempt ${attempt} failed:`, error.message);
            if (attempt === 3) {
...
async function handleGet(message, args, statusMsg) {
    ...
    try {
        ...
        // Tentar upload para Pastefy (não crítico)
        ...
        if (content.length > 1900) {
            // Se conteúdo for muito grande, enviar como arquivo
            const fileExt = path.extname(url).toLowerCase() || '.lua';
            ...
            // Se conteúdo for pequeno, mostrar no embed
            embed.setDescription(`\`\`\`lua\n${content}\n\`\`\``);
